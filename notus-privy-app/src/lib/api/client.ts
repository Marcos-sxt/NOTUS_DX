    ...
    beforeRequest: [
      (request) => {
        console.log('🔍 Notus API Request:', {
          ...
          headers: {
            'x-api-key': `${process.env.NEXT_PUBLIC_NOTUS_API_KEY?.slice(0, 20)}...`,
          },
        });
      },
    ],
...
console.log('🔧 Notus API Client Config:', {
  ...
  hasApiKey: !!process.env.NEXT_PUBLIC_NOTUS_API_KEY,
  apiKeyLength: process.env.NEXT_PUBLIC_NOTUS_API_KEY?.length || 0
});
